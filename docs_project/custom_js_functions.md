...
| **`f_form_reset(form_json)`** | Сбрасывает форму к исходным подписям, значениям и подсказкам из `control_arr`; пропускает `tag === 'html'`. Вызывает **`f_form_m_value`** (см. раздел «Замечания»). |
...
| **`f_ads_item_line_make(json_item)`** | Клонирует шаблон `ads_item_line`, заполняет ссылку, картинку, заголовок, цену, город, дату, класс избранного (`bi-heart` / `bi-heart-fill`). |
...
Скрипт оплаты Stripe (пример/заготовка на странице):

| Функция | Назначение |
|--------|------------|
| **`initialize()`** (async) | `fetch` к `create_payment_intent.php`, получает `clientSecret`, создаёт `stripe.elements`, монтирует Payment Element. |
| **`handleSubmit(e)`** (async) | Обработчик сабмита формы: `stripe.confirmPayment`, вывод ошибок через `showMessage`. |
| **`checkStatus()`** (async) | Если в URL есть `payment_intent_client_secret`, дергает `stripe.retrievePaymentIntent` и показывает статус платежа. |
| **`showMessage(messageText)`** | Показывает текст в `#payment-message`, скрывает через 4 с. |
| **`setLoading(isLoading)`** | Блокирует кнопку, переключает спиннер/текст кнопки. |

> PHP-функция **`create_payment_intent`** в том же файле — это **не JavaScript**, а серверный вызов API Stripe через cURL.
...
1. **`f_form_m_value`** вызывается из `f_form_reset`, но **объявления в репозитории не найдено** — при вызове `f_form_reset` возможна ошибка времени выполнения, пока функция не будет добавлена или вызов не уберут.
...
3. В **`showMessage`** в `user_pays_add.php` в `setTimeout` используется `messageText.textContent` — у строки нет `.textContent`; вероятная опечатка (имелся в виду `messageContainer`).
