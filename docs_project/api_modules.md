...
- **`expired` → `f_api_cron_expired`:** требуется совпадение параметра **`secret`** с **`WEB_JSON['api_json']['cron_secret']`** из **`config.php`** / **`config.defaults.php`**. Если секрет пустой в конфиге, вызов возвращает ошибку **`403 Forbidden`**.
